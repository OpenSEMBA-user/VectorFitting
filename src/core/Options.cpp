...
//vector<Complex> VectorFitting::poleIdentification(
//        const vector<Complex>& startingPoles) const {
...
//    //      f(s_k) = sample.second()
//
//    size_t N = startingPoles.size();
//
//    // Number of rows = number of samples
//    // Number of columns = 2* order of approximation + 2
//    MatrixXcd Dk(samplesSize_, 2*(N + 1));
//    MatrixXcd B( samplesSize_,             1);
//    for (size_t k = 0; k < samplesSize_; k++) {
//        Complex   s_k = samples_[k].first;
//
//        VectorXcd f_k = samples_[k].second;
//        B(k) = f_k;
//
...
//            // Real pole
//            if(isReal(a_i)){
//                Dk(k, i) = Complex(1.0, 0.0) / (s_k - a_i);
//                Dk(k, i + N + 2) = -Dk(k,i) * f_k;
//            }
//            // Complex pair
//            else{
//                // Sanity check
...
//
//                Dk(k,i) =   Complex(1.0, 0.0) / (s_k - a_i) + Complex(1.0, 0.0) / (s_k - conj(a_i));
//                Dk(k,i+1) = Complex(0.0, 1.0) / (s_k - a_i) - Complex(0.0, 1.0) / (s_k - conj(a_i));
//
//                Dk(k, i + N + 2)     = -Dk(k,i) * f_k;
//                Dk(k, i + N + 2 + 1) = -Dk(k,i+1) * f_k;
//
//                // FIXME: Ugly, beugh!
//                i++;
//            }
...
//
//    VectorXd sigmaResidues = X.tail(N);
//
...
//}
//
//void VectorFitting::residueIdentification(
//        const vector<Complex>& poles) {
